...
{
    public class DetailViewModel<T> : DataViewModelBase<T> where T : SchemaBase
    {
        private SectionConfigBase<T> _sectionConfig;
        private ComposedItemViewModel _selectedItem;
        private bool _isFullScreen;

        public DetailViewModel(SectionConfigBase<T> sectionConfig)
            : base(sectionConfig)
        {
        ...
        protected override void ParseItems(CachedContent<T> content, ItemViewModel selectedItem)
        {
            ...
            {
                SelectedItem = Items.FirstOrDefault(i => i._id == selectedItem._id);
            }
        }
